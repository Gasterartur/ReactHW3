...
const Rating = () => {
  ...
  return (
    <div style={{ textAlign: 'center', marginTop: '100px' }}>
      <img src={ratingList[ratingValue]} alt={`Rating ${ratingValue + 1}`} style={{ width: '200px', height: 'auto' }} />
      <div style={{ marginTop: '20px' }}>
        <button onClick={() => handleRatingChange(0)} style={buttonStyle}>Плохо</button>
        <button onClick={() => handleRatingChange(1)} style={buttonStyle}>Приемлемо</button>
        <button onClick={() => handleRatingChange(2)} style={buttonStyle}>Хорошо</button>
        <button onClick={() => handleRatingChange(3)} style={buttonStyle}>Отлично</button>
      </div>
    </div>
  );
};
